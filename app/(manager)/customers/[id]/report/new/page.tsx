...
interface NutritionResult {
  ...
  fat: number;       // 지방 (g)
  targetWeightLoss?: number; // 목표 감량 kg
  weeklyDeficit?: number;    // 주간 칼로리 적자
  weeksToGoal?: number;      // 목표 달성 예상 주
...
function calcNutrition(
  ...
  goal: string,
  bodyFatPct?: number,
  targetWeightLoss?: number  // ★ 목표 감량 kg 추가
): NutritionResult {
  // Mifflin-St Jeor 공식 (나이 30 기본값)
  ...
  if (targetWeightLoss && targetWeightLoss > 0) {
    // 1kg 지방 ≈ 7,700 kcal, 주 0.5~1kg 감량 권장
    const weeklyLossKg = Math.min(targetWeightLoss / 12, 0.8); // 12주 분배, 최대 0.8kg/주
    weeklyDeficit = Math.round(weeklyLossKg * 7700);
    const dailyDeficit = Math.round(weeklyDeficit / 7);
    targetKcal = Math.max(tdee - dailyDeficit, gender === 'female' ? 1200 : 1500); // 최소 섭취량 보장
  ...
  if (isLoss || targetWeightLoss) {
    mealPlan.push('🌅 아침: 닭가슴살 100g + 삶은 달걀 2개 + 방울토마토');
    mealPlan.push('☀️ 점심: 현미밥 반공기 + 두부조림 + 나물 반찬 2가지');
    mealPlan.push('🌙 저녁: 고구마 1개 + 그릭요거트 + 오이·당근 스틱');
    mealPlan.push('🍎 간식: 아몬드 15알 또는 단백질 쉐이크 (운동 후)');
    mealPlan.push('💧 수분: 하루 물 2L 이상 섭취');
  } else if (isBulk) {
    mealPlan.push('🌅 아침: 귀리오트밀 100g + 바나나 1개 + 삶은 달걀 3개');
    mealPlan.push('☀️ 점심: 현미밥 1공기 + 닭가슴살 150g + 채소볶음');
    mealPlan.push('🌙 저녁: 고구마 2개 + 두부 반모 + 브로콜리');
    mealPlan.push('💪 운동 후: 단백질 쉐이크 + 바나나 (30분 이내)');
    mealPlan.push('🌙 취침 전: 카제인 단백질 or 그릭요거트');
  } else {
    ...
    mealPlan.push('💧 수분: 하루 물 1.5L 이상 섭취');
  }
...
export default function NewReportPage() {
  ...
  const [targetWeightLoss, setTargetWeightLoss] = useState(''); // 목표 감량 kg
